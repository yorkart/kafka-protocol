    ...
    I::Error: ParseError<I::Token, I::Range, I::Position>,
{
    (
        ...
        array(|| {
            (string().expected("topic"), be_i32().expected("partition"))
                .map(|(topic, partition)| Partitions { topic, partition })
                .expected("partitions")
        }),
    )
        .map(
            |(controller_id, controller_epoch, delete_partitions, partitions)| StopReplicaRequest {
                controller_id,
                controller_epoch,
                delete_partitions,
                partitions,
